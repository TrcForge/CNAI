...
const [evidenceError, setEvidenceError] = useState('')
  ...
  const handleInvestigationSearch = async () => {
    const query = searchQuery.trim()

    if (!query) {
      setInvestigationData(null)
      setInvestigationError('Enter a criminal name, person ID, actor ID, or phone number.')
      return
    }
    ...
    try {
      const token = sessionStorage.getItem('access_token') || ''
      const data = await searchInvestigation(query, token)
      setInvestigationData(data)

      if (!data.found) {
        setInvestigationError(`No criminal record found for "${query}".`)
      }
    } catch (error) {
      console.error('Investigation search failed:', error)
      setInvestigationData(null)
      setInvestigationError(
        ...
        <div className="mb-5 flex min-h-[62px] items-center justify-between gap-4 rounded-xl border border-slate-200 bg-white px-5 py-2.5">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-0">
            ...
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter') {
                  void handleInvestigationSearch()
                }
              }}
            ...
            <button
              type="button"
              onClick={() => void handleInvestigationSearch()}
          ...
          <section className="space-y-8">
            ...
            {investigationLoading && (
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-5">
                <p className="font-semibold text-blue-900">Investigating...</p>
                <p className="mt-1 text-sm text-blue-800">
                  Correlating person, CDR, FIR, case, transaction, location and intelligence records.
                </p>
              </div>
            )}
            ...
            {investigationError && !investigationLoading && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-5">
                <p className="font-semibold text-amber-900">Investigation Search</p>
                <p className="mt-1 text-sm text-amber-800">{investigationError}</p>
              </div>
            )}

            {investigationData?.found && !investigationLoading && (
              <section className="rounded-xl border border-blue-200 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-2xl font-bold text-slate-900">
                      {investigationData.person?.person_name || investigationData.person?.person_id || investigationData.query}
                    </h3>
                    <p className="mt-1 text-sm text-slate-500">
                      Query: {investigationData.query}
                    </p>
                  </div>
                ...
                {investigationData.person && (
                  <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-4">
                    {[
                      ['Person ID', investigationData.person.person_id],
                      ['Actor ID', investigationData.person.actor_id],
                      ['Phone', investigationData.person.phone_number],
                      ['Case ID', investigationData.person.case_id],
                    ].map(([label, value]) => (
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
                  {[
                    ['CDR', investigationData.counts.cdr],
                    ['FIR', investigationData.counts.fir],
                    ['Cases', investigationData.counts.cases],
                    ['Transactions', investigationData.counts.transactions],
                    ['Locations', investigationData.counts.locations],
                    ['Phones', investigationData.counts.phones],
                    ['Vehicles', investigationData.counts.vehicles],
                    ['Intelligence', investigationData.counts.intelligence],
                  ].map(([label, value]) => (
                    <div key={label} className="rounded-lg border border-blue-100 bg-blue-50 p-4">
                      ...
                      <p className="mt-1 text-2xl font-bold text-blue-900">{value ?? 0}</p>
                    </div>
                  ))}
                </div>

                <div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <p className="text-sm font-semibold text-slate-800">Investigation scope</p>
                  <p className="mt-1 text-sm leading-6 text-slate-600">
                    Results are retrieved from the project's processed intelligence datasets and are intended to support investigator review. They do not establish guilt or criminal responsibility.
                  </p>
                </div>
              </section>
            )}
